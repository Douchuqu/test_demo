...
public class CharSegmentation {
	...
	public static Bitmap Math(int PlateX, int PlateY, Bitmap oriBmp) {
		...
		DrawP=0;
		byte size=0;
		PlateNumberGroup.PlateXState[K] = 0;
		for (X = 0; X < PlateX+1; X++)
		{
			if(Math.abs(GroupX[X] - MinJump)<10)
				{
					if(DrawP==1)
					{
						size++;
					}else
					{
						if(X!=0)
						{
							...
							K ++; PlateNumberGroup.PlateXState[K] = X;
							DrawP=1;size=1;
						}
						else
						{
		...
		for (X = 0; X < PlateX; X++) {
			...
			if (Math.abs(GroupX[X] - MinJump) < 10 ) {
				if (X - KN > 4) 
					DrawP = 1;
				else if ((X - KN >=2) && (Math.abs(GroupX[X] - GroupX[X - 1]) >= 1000)) 
					DrawP=1;
				if (DrawP == 1) {
					if (TX == 0) {
						if (X < 6) 
							...
							if (X > 15) {
								if (TX > (X- TX - 6) / 2) {
									...
									K ++; PlateNumberGroup.PlateXState[K] = TX; JG = X - TX -6; TX = X;
								}
							}
							canvas.drawLine(X, 0, X, PlateY, paint);
						...
						if (JG == 0) {
							canvas.drawLine(X, 0, X, PlateY, paint);
							K ++; PlateNumberGroup.PlateXState[K] = X; JG = X - TX - 6; TX = X;
						} else {
							if ((SZ == 0) && (X - TX > JG)) {
								canvas.drawLine(X, 0, X, PlateY, paint);
